...
# Attach dept name for readability
if "nom" in df_latest.columns:
    df_feat = df_feat.merge(df_latest[["dep", "nom"]].drop_duplicates(), on="dep", how="left")
...
STATIC_EXCL  = ["is_dom", "population"]   # kept for annotation, excluded from distance
FEATURE_COLS = [
...
for eps in DBSCAN_EPS:
    for min_s in DBSCAN_MIN_S:
        ...
        if n_clusters >= 2 and n_outliers < len(lbl) * 0.4:
...
df_dom = df_feat[df_feat["dep"].isin(DOM_DEPS)][
    ID_COLS + ["kmeans_cluster", "dbscan_cluster"]
].copy()

print(f"  DOM département cluster assignments (K-Means k={OPTIMAL_K}):")
for _, row in df_dom.iterrows():
    nm = row.get("nom", row["dep"])
    print(f"    {row['dep']} {nm:<20}  K-Means={row['kmeans_cluster']}  "
          f"DBSCAN={row['dbscan_cluster']}")

dom_clusters = df_dom["kmeans_cluster"].unique()
metro_only   = df_feat[~df_feat["dep"].isin(DOM_DEPS)]["kmeans_cluster"].unique()
dom_isolated = set(dom_clusters) - set(metro_only)

if dom_isolated:
    print(f"\n  ✓ DOM depts form their own cluster(s): {dom_isolated}")
    print("  ℹ️  Consider separating DOM from metropolitan clustering analysis")
else:
    print(f"\n  ℹ️  DOM depts distributed across metro clusters: {sorted(dom_clusters)}")
    print("  ✓ No dedicated DOM cluster — integration acceptable")
