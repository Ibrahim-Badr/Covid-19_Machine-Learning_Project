...
def load_sidep_dataset(dataset_id: str, label: str) -> pd.DataFrame:
    ...
    return pd.read_csv(best[0]["url"], sep=";", dtype=str, low_memory=False)
...
def load_finess_dataset() -> pd.DataFrame:
    ...
    for enc in ["utf-8", "latin1", "utf-8-sig", "cp1252"]:
        try:
            return pd.read_csv(url, sep=";", encoding=enc, dtype=str, low_memory=False)
        except UnicodeDecodeError:
            continue
    raise ValueError("Could not decode FINESS file with any encoding")
